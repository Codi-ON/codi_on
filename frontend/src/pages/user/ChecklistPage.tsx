...
const ChecklistPage: React.FC = () => {
  ...
  // ✅ selections가 완성되면 recoList 생성(=API 호출)해서 RecommendationPage로 넘길 데이터 준비
  useEffect(() => {
    let alive = true;
    ...
    const run = async () => {
      ...
      try {
        // TODO: 여기만 너희 API로 교체하면 됨
        // 예: POST /api/recommendation/closet-list
        const result = await requestRecoList(selections);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="space-y-1">
        ...
        {recoError && (
          <p className="text-sm font-bold text-red-500">{recoError}</p>
        )}
      </div>
 ...
 * TODO: 너희 백엔드로 교체
 * - 입력: selections
 * - 출력: RecommendationClosetList (top/bottom/outer 각 3개)
 */
async function requestRecoList(selections: Record<number, string>): Promise<RecommendationClosetList> {
  // 예시용 mock (현재 구조 깨지지 않게 타입만 맞춤)
  // 실제로는 fetch로 서버에서 받아오면 됨.
  void selections;
