...
public class SessionLogJdbcRepository {
    ...
    public void insert(SessionLogRequestDto dto) {
        String sql = """
                INSERT INTO session_log (user_id, session_key, event_type, payload)
                VALUES (:userId, :sessionKey, :eventType, CAST(:payload AS jsonb))
                """;
        ...
        jdbc.update(
                ...
                Map.of(
                        ...
                        "payload", dto.getPayloadJson()   // null이면 payload 컬럼도 null
                )
        );
    }
    ...
    public List<SessionLogResponseDto> findRecent(int limit) {
